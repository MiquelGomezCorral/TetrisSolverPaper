...
\keywords{
    Tetris; Optimització; Algorismes genètics; Metaheurístiques; Recobriment
} % Paraules clau 
{
   Tetris; Optimización; Algoritmos genéticos; Metaheurísticas; Enfriamiento simulado; Problema de cobertura
} % Palabras clave
{
    Tetris; Optimization; Genetic algorithms; Metaheuristics; Simulated annealing; Coverage problem
} % Key words
...
Este trabajo explora la resolución del juego de Tetris desde la perspectiva de la optimización, transformándolo en un problema de cobertura cuyo objetivo es minimizar el espacio ocupado por un conjunto predefinido de piezas. Para abordar este desafío combinatorio, se ha desarrollado una simulación de una versión moderna de Tetris y se han implementado dos algoritmos metaheurísticos: un Algoritmo Genético y un Enfriamiento Simulado.

La principal innovación de este enfoque radica en la codificación de las soluciones. En lugar de representar directamente las coordenadas finales de las piezas, cada posible solución se codifica como una secuencia de movimientos para cada una de las piezas disponibles. Este método garantiza la validez de todas las configuraciones generadas, ya que se obtienen a través de una simulación que respeta las reglas del juego. 

El estado final del tablero, o fenotipo, se evalúa mediante una función de aptitud compuesta. Dicha función no solo considera la puntuación del juego (líneas completadas), sino que también integra un conjunto de heurísticas diseñadas para medir la calidad estructural del tablero, como la altura, el número de agujeros y la rugosidad, junto con una penalización por movimientos ineficientes, etc.

A través de la optimización iterativa, los algoritmos buscan la secuencia de movimientos que conduce a la disposición más compacta de las piezas, demostrando la viabilidad de las técnicas metaheurísticas para resolver problemas complejos de planificación y optimización en entornos dinámicos y con restricciones como el Tetris, cuando una buena codificación para simular los individuos es implementada.
\end{abstract}
...
El Tetris, uno de los videojuegos más icónicos de la historia, ha trascendido su condición de mero entretenimiento para convertirse en un fascinante campo de estudio en la inteligencia artificial y la optimización. A primera vista, el objetivo es simple: encajar una secuencia de piezas geométricas que caen, llamadas tetrominos, para formar líneas horizontales completas que, una vez llenas, desaparecen. Sin embargo, detrás de esta aparente sencillez se esconde un problema combinatorio de gran complejidad.

Este trabajo aborda el Tetris no como un juego en tiempo real, sino como un problema de optimización: dado un conjunto finito de piezas, ¿cuál es la disposición óptima de estas en el tablero para minimizar el espacio total ocupado? Este enfoque transforma el juego en un problema de cubrimiento, donde el objetivo es encontrar la configuración más densa posible que deje con menos bloques el tablero final. Decimos menos bloques, porque como condiciones del juego, al completar una línea esta se limpia, lo que puede reducir el espacio ocupado.

Esta condición puede simular un problema real donde tras apilar ciertos elementos de una forma concreta, se consigue que 'estén listos para su eliminado', añadiendo más realismo al problema de optimización planteado y por supuesto, más complejidad.

Para resolver este problema, se exploran dos técnicas metaheurísticas: un Algoritmo Genético (GA) y un Enfriamiento Simulado (SA). La hipótesis de partida es que estos algoritmos, inspirados en la evolución natural y en procesos físicos de la materia, son capaces de navegar eficientemente por el espacio de soluciones para encontrar configuraciones de alta calidad.

Una de las claves de este proyecto reside en la codificación de las soluciones. En lugar de representar directamente las posiciones finales de las piezas, cada solución (o individuo) se codifica como una secuencia de movimientos para cada pieza. Este enfoque tiene una ventaja fundamental: garantiza que todas las soluciones generadas sean válidas según las reglas del juego, ya que se simula la colocación de cada pieza en el tablero siguiendo estos movimientos. El resultado de esta simulación, el estado final del tablero, constituye el fenotipo que será evaluado.

La evaluación de la calidad de cada solución se realiza mediante una función de aptitud diseñada para ser maximizada. Esta función no solo considera la puntuación obtenida en la partida, sino que también incorpora una penalización por movimientos ineficientes, y un conjunto de heurísticas que miden la "calidad" del tablero resultante, codificando el objetivo de minimización de la cobertura.

A lo largo de este documento, se detallará la codificación de los individuos, la formulación de la función objetivo y las heurísticas, para finalmente, analizar los resultados obtenidos por los algoritmos metaheurísticos en su tarea de resolver el problema de optimización planteado.
...
En este apartado se habla de como se han codificado los individuos para abordar el problema. Cabe recordar que el objetivo es encontrar, para un set de piezas concreto, la posición y orientación óptima de cada pieza para minimizar el espacio ocupado. A esto se deben añadir dos reglas del juego: primero que al completar una línea esta se limpia, y segundo que las piezas no pueden estar flotando al colocarse.
...
Al codificar los genotipos de esta forma, los individuos resultantes serán siempre válidos, ya que cada pieza se colocará en el tablero siguiendo las reglas del juego. Esto implica también que se debe ser capaz de simular el juego para poder evaluar cada individuo, \textbf{ya que el fenotipo de cada individuo será el estado final del tablero tras colocar todas las piezas siguiendo los movimientos indicados en el genotipo}.

También, habrá movimientos que resulten en '\textit{no-op}', como intentar mover una pieza a la izquierda cuando ya está en el borde izquierdo del tablero. Más adelante se muestra cómo se gestionan estos casos con la función objetivo.

\section{Movimientos posibles y tipos de genotipo}
Cuando se dice codificar los individuos como una secuencia de movimientos, es necesario definir cuales son estos. Si se observa el juego original, los movimientos posibles que puede hacer un jugador son:
...
En las versiones originales, cuando se dejaba caer una pieza y esta toca el suelo, se bloqueaba al instante. Sin embargo, en versiones más modernas esto no es así y aún habiendo tocado el suelo, se permite mover la pieza con unas ciertas reglas. En este caso, se estaría hablando de un Tetris moderno como el Tetris 99 \cite{tetris99-tetriscompany-no-date}, donde se permite que las piezas se muevan con más libertad y por tanto, se da más capacidad de representación al jugador. 
...
Ahora, con cierta experiencia en el juego, se puede apreciar que no en todas las situaciones todos los movimientos son necesarios para llegar a una solución concreta. Sin ir más lejos, en casos donde el tablero está casi vacío y no hay piezas creando agujeros, siempre se podrá poner una pieza en cualquiera de las posiciones validas con una \textit{rotación} y un \textit{movimiento lateral}.
...
También cabe mencionar que cuando una pieza termine de ser movida se bloqueará al momento, dejándola caer hasta tocar el suelo u otra pieza y añadiéndola al tablero.

En la siguiente figura se muestran los movimientos posibles para los genotipos SwapSimple y SwapDouble. Aquí se pueden apreciar situaciones como en las de la derecha, donde la pieza azul de la izquierda puede aprovechar esos movimientos dobles para introducirse en la 'cueva' que crear la otra 'pseudo pieza'.

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/movements_simple.png}
        \caption{Movientos SwapSimple}
        \label{fig:movements_simple}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/movements_double.png}
        \caption{Movientos SwapDouble}
        \label{fig:movements_double}
    \end{subfigure}
    ...
    % \end{subfigure}
    \caption{Esquema visual de movimientos posibles en genotipos SwapSimple y SwapDouble.}
    \label{fig:pieces_movements}
\end{figure}


\section{Talla del problema}
En este trabajo se considerarán tres tallas del problema diferentes, con 10, 20 y 30 piezas siendo estás consideradas como fácil, media y difícil respectivamente.

Ahora, al definir los movimientos de la forma considerada, se ha de tener en cuenta la explosión combinatoria que se genera. Si se tiene un set de piezas de tamaño $N$ y cada pieza se tiene distintos movimientos $M_1, M_2, \ldots, M_k$ cada uno con su espacio, el espacio de búsqueda total $S$ se puede calcular como, el producto de todas las posibles combinaciones de movimientos para cada pieza, elevado al número de piezas:

\begin{align*}
S = (M_1 \times M_2 \times ... \times M_k)^N
\end{align*}

Por ejemplo, para ver el tamaño con un set de movimientos \texttt{Simple} y \texttt{SwapDouble} (2 y 5 movimientos posibles) y un set de piezas de tamaño 10, el espacio de búsqueda sería:
\begin{align*}
S_{Simple,10} &= (11 \times 4)^{10} = 44^{10} \approx 2.72 \times 10^{16} \\
S_{SwapDouble,10} &= (2 \times 11 \times 4 \times 19 \times 4)^{10} = 6688^{10} \approx 1,79 \times 10^{38}
\end{align*}

Ahora, en el caso de tallas más complejas, como un set de 30 pieazas, el espacio de búsqueda se vuelve inmanejable:
\begin{align*}
S_{Simple,30} &= (11 \times 4)^{30} = 44^{30} \approx 2.01 \times 10^{49} \\
S_{SwapDouble,30} &= (2 \times 11 \times 4 \times 19 \times 4)^{30} = 6688^{30} \approx 5.94 \times 10^{114}
\end{align*}

Como referencia, el número estimado de átomos en el universo observable es aproximadamente $10^{80}$ \cite{cuantosatomos-rol-2019}. Por lo tanto, incluso con un set de piezas relativamente pequeño, el espacio de búsqueda se vuelve inmanejable para una búsqueda exhaustiva. Esto justifica el uso de técnicas metaheurísticas como los Algoritmos Genéticos y el Enfriamiento Simulado para explorar eficientemente este vasto espacio de soluciones.

También, cabe destacar que al usar este tipo de codificación, muchas de estas combinaciones resultarán en el mismo tablero final, ya que muchos movimientos pueden resultar en '\textit{no-op}' o movimientos que no cambian el estado final del tablero. Esto introduce redundancia en el espacio de búsqueda, pero sigue siendo preferible a generar soluciones inválidas.
...
Esta función se pretende \textbf{MAXIMIZAR}. A continuación se desglosa cada uno de los términos:
\begin{itemize}
    \item \textbf{Puntuación juego}: es la puntuación obtenida tras jugar la partida con el genotipo indicado. Esta puntuación se calcula como en el juego original, donde se otorgan puntos por cada línea completada, y se otorgan puntos extra por completar varias líneas a la vez (doble, triple, tetris) \cite{lineclear-harddropwiki-no-date}. Se omiten puntos extra por combos o por dejar caer las piezas rápidamente, ya que no aportan nada a la calidad de la solución. También se omiten los \texttt{T-spins} \cite{tspin-tetriswiki-no-date}, de nuevo por no aportar nada a la calidad de la solución.
...
Se han definido las siguientes heurísticas para evaluar la calidad del tablero final. El origen de todas ellas es una mezcla entre heurísticas clásicas usadas en la literatura para jugar al Tetris con IA \cite{tetrisgenetic-lewis-2015}, videos de youtube \cite{tetrisai-uci-2020} y experiencia personal. La idea principal tras estas, es codificar el objetivo de 'minimización' del espacio ocupado y guiar la búsqueda hacia soluciones buenas.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/heuristics.png}
    \caption{Esquema visual de heuristicas: Rugosidad, Agujeros por columna, bloques, y bloques sobre agujeros.}
...
Los factores que ponderan cada uno de los términos de la función objetivo, se han establecido tras una serie de pruebas preliminares, referencias en la literatura \cite{tetrisgenetic-lewis-2015} y 'a ojo'. Lo ideal sería poder lanzar una serie de experimentos para ajustar estos pesos, pero por limitaciones computacionales y de 'sentido' no ha sido posible.
...
\begin{itemize}
    ...
    \item Con los factores puestos a ojo, y las diferentes heurísticas, se considera que un individuo es 'bueno' si su fitness supera los $-50$ puntos.
\end{itemize}
...
El desarrollo del proyecto se ha realizado mayoritariamente en C\#. Para lo que sería la parte visual se ha utilizado el framework Unity \cite{unityplatform-unitytechnologies-no-date}, que permite crear aplicaciones gráficas de forma sencilla y rápida. También se ha usado Python para la visualización de resultados y generación de gráficos.
...
Por último, para la visualización de estos resultados, su análisis y generación de gráficos, se ha usado Python con librerías como Matplotlib \cite{matplotlib-org-no-date} y Plotly \cite{plotlydata-plotly-no-date}. Estas librerías permiten crear gráficos de forma sencilla y personalizable, lo que facilita la presentación de los resultados obtenidos en los experimentos.

La Implementación de todo el código se puede encontrar en el repositorio de GitHub \cite{tetrissolver-miquelgomezcorral-no-date}.

\section{Implementación del Tetris}
Como ya se ha mencionado, la versión del Tetris utilizada es una versión moderna. Para tener control absoluto del juego y su simulación, se ha implementado el juego desde cero usando la guía oficial para el desarrollo de juegos Tetris \cite{tetrisguideline-blueplanet-2009}.
...
El algoritmo genético (Genetic Algorithm, GA) \cite{geneticalgorithm-alam-2020} se ha implementado siguiendo los principios básicos de esta técnica de optimización. Siguiendo lo mencionado anteriormente, se habla de como se llevan a cabo los pasos del algoritmo, en concreto: de la Población inicial, el Cruce, la Mutación, la Selección y el Reemplazo.
...
A todos los individuos se les asigna una probabilidad usando la función Softmax \cite{softmax-wikipedia-2024}. Esta función transforma los valores de fitness en probabilidades entre 0 y 1, de forma que la suma de todas las probabilidades sea 1.
...
Como se puede observar en la gráfica, siempre se usan valores muy elevados de temperatura. A lo largo de la memoria se hará más incapié en este punto, pero la EXPLORACIÓN es lo que más ha beneficiado a la generación de buenas soluciones. En esa línea se seleccionan más parámetros en los siguientes puntos. 
...
Cabe destacar que además, a la hora de evaluar las fitness de la nueva generación y ordenar los individuos, se ahorra la mitad de los cálculos al haber mantenido la mitad de la generación anterior. Esto es, solo se evalúan los $N/2$ nuevos individuos y se combinan con los $N/2$ individuos de la generación anterior, cuyos fitness ya se conocen.


\section{Enfriamiento simulado}
De forma similar a como se ha hecho en el apartado anterior, el Enfriamiento simulado (Simulated Annealing, SA) \cite{simulatedannealing-henderson-2006} se ha implementado siguiendo los principios básicos de esta técnica de optimización. 
...
Una vez se tiene este individuo inicial, es necesario definir cómo se generan soluciones vecinas a partir de él.
...
Ahora, no se puede siempre decrementar este valor, hay que ser conscientes de si el algoritmo se está quedando estancado o no. La forma de medir este estancamiento se controla mediante una 'paciencia'. 
...
\subsection{Notas}
Cabe comentar ciertos aspectos importantes a modo de resumen sobre el enfriamiento simulado:
\begin{itemize}
...
De esta forma, se asegura visitar todos los vecinos al menos una vez antes de considerar que el algoritmo está estancado y aumentar la temperatura.
...
En este caso, la introducción del \texttt{swap} parece tener efecto en los resultados pero es tan severo como la introducción del \texttt{double}. Siendo que ha sido el único tipo de movimientos que ha permitido encontrar soluciones decentes para este caso, la expresividad del \texttt{SwapDouble} es clave.
...
A continuación se analizarían los resultados obtenidos al variar cada uno de estos parámetros. Sin embargo, lo que se ha observado es que \textbf{ninguno de los parámetros propios del algoritmo, afecta de forma significativa a la calidad de las soluciones obtenidas}.
...
Ahora, esto solo sucede con tallas pequeñas del problema, ya que en casos como las 30 piezas, ningún tipo de individuo consigue soluciones aceptables. El único caso en el que se ha observado posibilidad de convergencia hacía soluciones aceptables, ha sido con el tipo \texttt{SwapDouble}.
...
A continuación se analiza la evolución de ambos algoritmos para observar este comportamiento, pero como se puede intuir, el enfriamiento simulado no parece ser el algoritmo más adecuado para este problema.
...
Se observa que la mayoría de estas ejecuciones no dejan de mejorar poco a poco hasta el final, lo que indica que el algoritmo sigue explorando y mejorando la solución a lo largo de toda la ejecución.

\textbf{Mejor solución encontrada:} Si se observa la gráfica, se puede apreciar una ejecución de 10 piezas que consigue una solución por encima de $0$. Cabe mencionar que esta ha sido la única ejecución que ha conseguido llegar al objetivo ideal del problema, limpiando todo el tablero sin dejar ningún bloque.
